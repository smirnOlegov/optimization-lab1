...
int main() {
    ...
    auto print_benchmark = [](const TimedRun& run, double target_value) {
    ...
    TimedRun gd_2d = run_benchmark("Gradient Descent", [&] {
        GradientDescentOptimizer gd(start_2d, OptimizationType::MINIMIZE, 0.001, 200000);
        return gd.optimize(Rosenbrock2D, 2, rosenbrock_target, tolerance_2d);
    });
    ...
    TimedRun ga_2d = run_benchmark("Genetic Algorithm", [&] {
        ...
        return ga.optimize(Rosenbrock2D, 2, rosenbrock_target, tolerance_2d);
    });
    print_benchmark(ga_2d, rosenbrock_target);

    TimedRun graal_2d = run_benchmark("Accel GRAAL", [&] {
        AcceleratedGRAALOptimizer graal(start_2d, OptimizationType::MINIMIZE, 200000, 0.001);
    ...
    std::cout << "Target value: " << rosenbrock_target
              << ", tolerance: " << tolerance_nd << "\n\n";

    TimedRun gd_nd = run_benchmark("Gradient Descent", [&] {
        GradientDescentOptimizer gd(start_nd, OptimizationType::MINIMIZE, 0.0005, 300000);
    ...
    TimedRun ga_nd = run_benchmark("Genetic Algorithm", [&] {
        GeneticAlgorithmOptimizer ga(OptimizationType::MINIMIZE, 500, 10000);
        return ga.optimize(RosenbrockND, 5, rosenbrock_target, tolerance_nd);
    });
    print_benchmark(ga_nd, rosenbrock_target);

    TimedRun graal_nd = run_benchmark("Accel GRAAL", [&] {
        AcceleratedGRAALOptimizer graal(start_nd, OptimizationType::MINIMIZE, 200000, 0.001);
        return graal.optimize(RosenbrockND, 5, rosenbrock_target, tolerance_nd);
    });
    print_benchmark(graal_nd, rosenbrock_target);

    print_iteration_comparison("Rosenbrock ND (N=5)", {gd_nd, ga_nd, graal_nd});
